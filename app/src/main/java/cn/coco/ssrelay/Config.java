...
final class Config {
    final String host, user, bind, localBind, method, password, fingerprint;
    final int sshPort, remotePort, localPort;
    Config(String host, int sshPort, String user, int remotePort, int localPort,
           String bind, String localBind, String method, String password, String fingerprint) {
        ...
        this.password=password; this.fingerprint=fingerprint;
    }
    ...
    static Config load(Context c) {
        ...
        return new Config(p.getString("host",""), p.getInt("sshPort",22),
                ...
                p.getString("password",""), p.getString("fingerprint",""));
    }
    void save(Context c) {
        prefs(c).edit().putString("host",host).putInt("sshPort",sshPort).putString("user",user)
                ...
                .putString("password",password).putString("fingerprint",fingerprint).apply();
    }
    void validate() {
        ...
        if(password.isEmpty()) throw new IllegalArgumentException("请在“密钥”页设置 SS 密码");
        if(fingerprint.trim().isEmpty()) throw new IllegalArgumentException("请在“密钥”页填写 SSH 主机指纹");
